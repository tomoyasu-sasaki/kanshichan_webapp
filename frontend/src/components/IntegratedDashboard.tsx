  ...
  const renderOverallHealth = () => {
    ...
    return (
      <Card bg={cardBg}>
        <CardHeader>
          <HStack justify="space-between">
            <Heading size="md">システム全体健康度</Heading>
        ...
        <CardBody>
          <VStack spacing={4}>
            ...
            {/* 監視常時アクティブ状態表示 */}
            <HStack justify="center" width="100%" p={3} bg="green.50" borderRadius="md">
              <Icon as={FiActivity} color="green.500" />
      ...
      default:
        return (
          <VStack spacing={6} align="stretch">
            <Text fontSize="lg" fontWeight="bold">KanshiChan 統合ダッシュボード</Text>
            <Text color="gray.600">
              各フェーズの機能が正常に動作しています。左側のメニューから詳細画面にアクセスできます。
            </Text>
            {renderSystemStatus()}
            {renderOverallHealth()}
          </VStack>
  ...
  return (
    <Box bg={bgColor} minH="100vh">
      <Grid templateColumns="250px 1fr" gap={6}>
        {/* Sidebar Navigation */}
        <GridItem>
          <Box position="fixed" height="100vh" width="250px" bg={cardBg} p={4} overflowY="auto">
            <VStack spacing={3} align="stretch">
              <Heading size="md" mb={4}>KanshiChan</Heading>
