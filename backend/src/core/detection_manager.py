# backend/src/core/detection_manager.py
    ...
    def detect(self, frame: np.ndarray) -> List[Dict[str, Any]]:
        ...
        try:
                ...
                for label, detections in detector_results['detections'].items():
                    ...
                    for det in detections:
                         ...
                         unified_detections_list.append({
                             ...
                             'box': det.get('bbox') # detector.py の bbox はタプル (x1, y1, x2, y2)
                         })
            ...
            if detector_results.get('pose_landmarks'):
                landmarks_data['pose'] = detector_results.get('pose_landmarks')
            if detector_results.get('hands_landmarks'):
                landmarks_data['hands'] = detector_results.get('hands_landmarks')
            if detector_results.get('face_landmarks'):
                landmarks_data['face'] = detector_results.get('face_landmarks')
            
            # いずれかのランドマークが存在すれば追加
            if landmarks_data:
                unified_detections_list.append({'label': 'landmarks', 'data': landmarks_data})
